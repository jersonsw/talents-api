...
export const ApiUnpaginatedRespDoc = <TModel extends Type>(props: Props<TModel>) => {
  const additionalApiDecorators = props.additionalApiDecorators || [];
  const entityName = props.entityName || 'Element';
  const model = props.model;
  const pronoun = 'The';
  const summary = props.summary || `Gets a list of ${entityName.toLowerCase()}`;
    ...
    ApiForbiddenResponse({
      ...
      schema: {
        properties: {
          ...
          message: {
            type: 'string',
            example: `Don't have access to get ${pronoun.toLowerCase()} ${entityName.toLowerCase()}`,
          },
          timestamp: {
            type: 'number',
            example: Date.now(),
